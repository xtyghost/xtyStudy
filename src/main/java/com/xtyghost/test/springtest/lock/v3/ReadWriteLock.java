...
public class ReadWriteLock {
    ...
    public synchronized void lockWriter() throws InterruptedException {
        ...
        while ((writerThread !=null&&writerThread!=Thread.currentThread() )|| readerMap.size() > 0) {
            wait();
        }
        /**
         * 占据写权利
    ...
    public synchronized void unlockWriter() throws InterruptedException {
        while (Thread.currentThread()!=writerThread|| readerMap.size() > 0) {
            wait();
        }
        writer--;
        if (writer==0){
    ...
    public synchronized void lockReader() throws InterruptedException {
        while (writer > 0 || writerThread !=null) {
            wait();
        }
